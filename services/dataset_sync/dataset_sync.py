...
def run_conversion(config, commit_message_metadata: str):
    ...
    'config_manager' is the configuration object loaded by dataset_sync.py.
    ...
    convert_script_name = config[
        "CONVERT_SCRIPT"
    ]  # From config_manager, e.g., "convert_raw_annotations.py"
    ...
    if not env_file_for_conversion:
        logging.error(
            "ENV_FILE not found in config_manager. Cannot run conversion script."
        )
    ...
    command = [
        ...
        convert_script_name,
        "--env_file",
        str(env_file_for_conversion),
        "--commit_message_metadata",
        str(commit_message_metadata),  # Ensure it's a string
    ]

    logging.info(
        f"Running conversion script with real-time logging: {' '.join(command)}"
    )
    ...
    try:
        ...
        if return_code == 0:
            ...
            raise subprocess.CalledProcessError(
                return_code, command, output=None, stderr=None
            )

    except FileNotFoundError:
        logging.error(
            f"Conversion script '{convert_script_name}' not found. Ensure it is in the correct path."
        )
        raise
    except Exception as e:
        logging.error(
            f"An unexpected error occurred while running the conversion script: {e}"
        )
        raise
...
def compute_state_hash(state: dict) -> str:
    return sha1(
        json.dumps(strip_noncritical(state, "file"), sort_keys=True).encode()
    ).hexdigest()
...
def sync_to_directory(client, state: dict, config) -> dict:
    ...
    for obj_name, obj_info in state.items():
        annotation_object = client.get_object(config["MINIO_BUCKET"], obj_name)

        annotation_json = json.loads(annotation_object.read().decode("utf-8"))
        local_identifier = s3_path_to_identifier(
            Path(annotation_json["task"]["data"]["image"])
        )

        json_path = Path(config["LS_ANNOTATIONS_DIR"]) / Path(
            local_identifier + ".json"
        )
        ...
        if not json_path.exists():
            ...
            with open(json_path, "r") as f:
                ...
                if compute_data_hash(annotation_json) != compute_data_hash(
                    existing_annotation
                ):
                    logging.info(
                        f"Updating annotation for {local_identifier} in {json_path}"
                    )
                    with open(json_path, "w") as f:
                        json.dump(annotation_json, f, indent=4)
                    ...
                    logging.info(
                        f"Annotation for {local_identifier} already exists, skipping."
                    )
...
def sync_once(client, config):
    ...
    if old_state_hash == new_state_hash:
        ...
        commit_message_metadata = (
            f"new: {len(new_keys)}, modified: {modified_count}, total: {len(new_state)}"
        )
        ...
        if modified_count + len(new_keys) >= config["MIN_NEW"]:
            logging.info(
                f"Triggering workflow for {modified_count + len(new_keys)} modified files."
            )
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Sync MinIO bucket and convert annotations."
    )
    parser.add_argument(
        "--loop",
        action="store_true",
        help="Run in loop mode, syncing every POLL_INTERVAL seconds.",
    )
    parser.add_argument(
        "--env_file",
        type=str,
...
def main():
    ...
    if cli_args.loop:
        logging.info(
            f"Running in loop mode. Polling every {config['POLL_INTERVAL']} seconds. Press Ctrl+C to stop."
        )
