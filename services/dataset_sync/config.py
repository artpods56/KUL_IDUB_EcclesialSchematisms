...
def load_config_from_env(env_file: str) -> dict:
    ...
    if env_file and Path(env_file).exists():
        ...
        print(
            f"Environment file '{env_file}' not found or not specified. Using system environment variables and defaults."
        )
    ...
    config = {
        ...
        "CONVERT_SCRIPT": os.getenv(
            "CONVERT_SCRIPT", "convert_raw_annotations.py"
        ),  # This is a script description, not a path to manage here
        "IMAGE_DIR": get_path_env("IMAGE_DIR", "./images"),
        "LS_ANNOTATIONS_DIR": get_path_env(
            "LS_ANNOTATIONS_DIR", "./label_studio_annotations"
        ),
        "OUT_JSONL": get_path_env(
            "OUT_JSONL", "./train.jsonl"
        ),  # This will be a Path object
    ...
    print(
        f"Loaded config - MinIO endpoint: {config['MINIO_ENDPOINT']}, bucket: {config['MINIO_BUCKET']}"
    )
    # Example of how a path is now a Path object
    if config["IMAGE_DIR"]:
        print(
            f"Image directory (as Path object): {config['IMAGE_DIR']}, type: {type(config['IMAGE_DIR'])}"
        )
    return config
