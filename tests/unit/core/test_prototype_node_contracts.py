...
from notarius_core.prototype.nodes import InPort, Node, NodeExecutionContext, OutPort
from notarius_core.prototype.nodes import PortShape, derive_input_contract
...
class ExampleInput(NodeInput):
    pages: Annotated[list[Image], InPort(SOURCE_PAGE_IMAGE)]
...
class AnnotationShapesInput(NodeInput):
    optional_page: Annotated[Image | None, InPort(SOURCE_PAGE_IMAGE)] = None
...
def test_node_contracts_derive_shape_and_resolver_target_from_annotations() -> None:
    ...
    assert ExampleNode.input_contract.ports["pages"].target_type is Image
...
def test_input_contract_supports_optional_variadic_and_structural_types() -> None:
    ...
    assert optional_page.target_type is Image
    ...
    assert page_batches.target_type is Image
