...
@pytest.fixture
def arithmetic_client(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> TestClient:
    service = WorkbenchService(workspace=tmp_path / "workbench")
    monkeypatch.setattr(
        prototype_routes,
...
def test_registry_declares_generic_integer_and_arithmetic_result_projections(
    arithmetic_client: TestClient,
) -> None:
    response = arithmetic_client.get("/v1/prototype/nodes")

    assert response.status_code == 200
    registry = PrototypeNodeRegistryResponse.model_validate(response.json())
    artifact_types = {
        artifact_type.key.id: artifact_type
    ...
    assert [
        (nodes[operator_id].title, nodes[operator_id].group)
        for operator_id in (
            "arithmetic.number",
...
def test_arithmetic_graph_projects_both_result_fields_into_multiply(
    ...
    response = arithmetic_client.post(
        "/v1/prototype/run",
    ...
    compound_content = arithmetic_client.get(
        f"/v1/prototype/artifacts/{compound.artifact_id}/content"
    )
    ...
    product_content = arithmetic_client.get(
        f"/v1/prototype/artifacts/{product.artifact_id}/content"
    )
