...
from notarius_core.prototype.materialization import MaterializationProvenance
...
from notarius_core.prototype.ocr import OcrPagePayload, SimpleOcrResult
from notarius_core.prototype.sources import SourceImageContent
from notarius_core.prototype.tables import TableCsvBundle
...
class OcrPageResultOutputWriter(ArtifactOutputWriter):
    ...
    async def write(
        ...
        payload_hash = _content_hash(payload_json)
        artifact = ArtifactObject(
            artifact_type=OCR_PAGE_RESULT.key.id,
            schema_version=OCR_PAGE_RESULT.key.schema_version,
            content_type="application/json",
            storage_backend="inline",
    ...
    def _payload_from_value(
        ...
        item_index = context.item_index if context.item_index is not None else 0
        if item_index >= len(source_refs):
            raise RuntimeError("OCR output does not have a matching source page ref")

        source_ref = source_refs[item_index]
