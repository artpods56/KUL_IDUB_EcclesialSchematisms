...
class SchematismEntryWithValue(SchematismEntry):
    value: str | None = None
...
class SchematismWithValuePage(SchematismPage):
    entries: list[SchematismEntryWithValue] = Field(default_factory=list)


class CleryEntry(SchematismEntry):
    pass


class ElenchusPageContext(PageContext):
    pass


class ElenchusPage(BaseModel):
    model_config = ConfigDict(extra="forbid")

    page_number: str | None = Field(None, description="Page number as string")
    entries: list[CleryEntry] = Field(default_factory=list)
    context: ElenchusPageContext | None = None
