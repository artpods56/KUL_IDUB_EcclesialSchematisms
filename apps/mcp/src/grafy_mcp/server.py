...
from grafy_mcp.operations import McpOperationError
from grafy_mcp.request_context import current_mcp_binding
...
def _as_tool_error(error: McpOperationError, operation: str) -> ToolError:
    ...
    if error.status_code == 404:
        return ToolError(
            f"The requested Grafy resource was not found while attempting to "
            f"{operation}."
        )
