...
- Reviewed: the Grafy workbench backend — `apps/api` (FastAPI service), `apps/mcp` (MCP server), `libs/core` (ports, domain, application, runtime), `libs/persistence` (SQL adapters), `libs/storage`, and `plugins/` (llm, sql, ocr, gis). ~197 Python source files; ~46,260 lines.
