...
def run_git_command(command: List[str], working_dir: Path, raise_on_error: bool = True) -> Optional[subprocess.CompletedProcess]:
    ...
    try:
        ...
        logging.error(f"An unexpected error occurred while running git command {' '.join(command)}: {e}")
        if raise_on_error:
            raise
        return None
...
def is_repo_dirty(repo_dir: Path) -> bool:
    ...
    process = run_git_command(["git", "status", "--porcelain"], repo_dir, raise_on_error=False)
...
def push_to_hf_repo(config, commit_message_metadata = ""):
    """Push the generated train.jsonl to HuggingFace dataset repository using direct git commands."""
    ...
    try:
        ...
        if not hf_repo_dir.is_dir() or not (hf_repo_dir / ".git").is_dir():
            logging.error(f"HF_REPO_DIR {hf_repo_dir} is not a valid git repository. Skipping push.")
        ...
        if lock_file.exists():
            ...
            try:
                ...
                logging.error(f"Failed to remove lock file {lock_file}: {e}. Git operations might fail.")
                # Not raising here, to allow git commands to try and potentially provide more specific errors

        # 1. Git Pull
        logging.info(f"Pulling latest changes from remote repository in {hf_repo_dir}...")
        ...
        try:
            ...
            status_result = run_git_command(["git", "status", "-s"], hf_repo_dir, raise_on_error=False)
            if status_result:
        ...
        if not full_output_file_path.exists():
            logging.error(f"Output file {full_output_file_path} does not exist. Cannot add to git.")
        ...
        if is_repo_dirty(hf_repo_dir):
            ...
            #    # but assumes 'origin' is the remote name and needs careful handling of the URL.
            ...
            logging.info(f"Successfully pushed changes to HF repository: {config['HF_REPO_URL']}")
        else:
        ...
        logging.error(f"An unexpected error occurred in push_to_hf_repo: {e}", exc_info=True)
        raise
